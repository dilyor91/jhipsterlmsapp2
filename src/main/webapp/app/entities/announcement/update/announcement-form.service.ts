...
type AnnouncementFormDefaults = Pick<NewAnnouncement, 'id' | 'delayPost' | 'postAt' | 'published' | 'announcements'>;
...
type AnnouncementFormGroupContent = {
  ...
  announcements: FormControl<AnnouncementFormRawValue['announcements']>;
};
...
export class AnnouncementFormService {
  createAnnouncementFormGroup(announcement: AnnouncementFormGroupInput = { id: null }): AnnouncementFormGroup {
    ...
    return new FormGroup<AnnouncementFormGroupContent>({
      ...
      announcements: new FormControl(announcementRawValue.announcements ?? []),
    });
  }
