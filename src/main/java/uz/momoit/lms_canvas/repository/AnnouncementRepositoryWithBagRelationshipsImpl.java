...
public class AnnouncementRepositoryWithBagRelationshipsImpl implements AnnouncementRepositoryWithBagRelationships {
    ...
    public Optional<Announcement> fetchBagRelationships(Optional<Announcement> announcement) {
        return announcement.map(this::fetchAnnouncements);
    }
    ...
    public List<Announcement> fetchBagRelationships(List<Announcement> announcements) {
        return Optional.of(announcements).map(this::fetchAnnouncements).orElse(Collections.emptyList());
    }

    Announcement fetchAnnouncements(Announcement result) {
        return entityManager
            .createQuery(
                "select announcement from Announcement announcement left join fetch announcement.announcements where announcement.id = :id",
                Announcement.class
            )
    ...
    List<Announcement> fetchAnnouncements(List<Announcement> announcements) {
        ...
        List<Announcement> result = entityManager
            .createQuery(
                "select announcement from Announcement announcement left join fetch announcement.announcements where announcement in :announcements",
                Announcement.class
            )
